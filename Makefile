...
res: gen/FogpadEditRes.cpp

gen/FogpadEditRes.cpp: resources/Fogpad.json utils/bin/rescc$(APP_EXT)
	@install -d gen
...
clean:
	...
	$(MAKE) clean -C plugins/Fogpad
	rm -rf bin build gen

install: all
	$(MAKE) install -C plugins/Fogpad

install-user: all
	$(MAKE) install-user -C plugins/Fogpad
